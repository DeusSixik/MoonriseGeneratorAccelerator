...
import dev.sixik.msplit.Splitter;
...
import static dev.sixik.msplit.Util.OBJECT_TYPE;
import static dev.sixik.msplit.Util.boxStackIfNecessary;
import static dev.sixik.msplit.Util.boxedTypeIfNecessary;
import static dev.sixik.msplit.Util.intConst;
import static dev.sixik.msplit.Util.isStoreOp;
import static dev.sixik.msplit.Util.loadOpFromType;
import static dev.sixik.msplit.Util.storeOpFromType;
import static dev.sixik.msplit.Util.unboxStackIfNecessary;
import static dev.sixik.msplit.Util.zeroVal;
...
public class SplitMethod {
    ...
    protected MethodNode createSplitOffMethod(MethodNode orig, Splitter.SplitPoint splitPoint) {
        ...
        // Now go over each stack item and load the arr, swap w/ the stack, add the index, swap with the stack, and
        // store
    ...
    protected MethodNode createTrimmedMethod(String owner, MethodNode orig,
        ...
        for (int i = 0; i < splitPoint.start; i++) {
            ...
            if (insn instanceof VarInsnNode && isStoreOp(insn.getOpcode()))
                seenStoredLocals.add(((VarInsnNode) insn).var);
            insn.accept(newMethod);
        }
