...
public class Splitter implements Iterable<Splitter.SplitPoint> {
    ...
    @Override
    public Iterator<SplitPoint> iterator() {
        return new Iter();
    }
    ...
    public static class SplitPoint {
        ...
        /**
         * The locals written in this split area, keyed by index. Value type is always int, float, long, double, or
         * object.
         */
        ...
        /**
         * The values of the stack at the end of this split area that are needed to put back on the original. Type is
         * always
    ...
    protected class Iter implements Iterator<SplitPoint> {
        ...
        protected SplitPoint longestForCurrIndex() {
            ...
            // Now that we have our largest range from the start index, we can go over each updating the local refs
            // and stack
        ...
        protected void constrainEndByTryCatchBlocks(InsnTraverseInfo info) {
            ...
            for (TryCatchBlockNode block : tryCatchBlocks) {
                ...
                } else if (info.startIndex <= startIndex) {
            ...
            for (int i = start; i < stack.size(); i++) {
                ...
                if (item == Opcodes.LONG || item == Opcodes.DOUBLE) {
                    if (stack.get(++i) != Opcodes.TOP)
                        throw new IllegalStateException("Expected top after long/double");
                }
            }
            return ret;
        }
    ...
    protected static class StackAndLocalTrackingAdapter extends AnalyzerAdapter {
        ...
        protected StackAndLocalTrackingAdapter(Splitter splitter) {
            super(splitter.api, splitter.owner, splitter.method.access, splitter.method.name, splitter.method.desc,
                    null);
            stack = new SizeChangeNotifyList<Object>() {
                @Override
                protected void onSizeChanged() {
                    lowestStackSize = Math.min(lowestStackSize, size());
                }
            };
        }
