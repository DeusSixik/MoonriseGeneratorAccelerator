...
import dev.sixik.generator_accelerator.common.density.density_custom.pure.FastClampDensityFunction;
...
public class DensityOptimizer {
    ...
    private DensityFunction rewriteLocal(DensityFunction function) {
        ...
        if (unwrapped instanceof FastClampDensityFunction clamp) {
        ...
        if (unwrapped instanceof DensityFunctions.TwoArgumentSimpleFunction ap2) {
             ...
             */
            return DensityCustomFunction.createFastVersion(type, aUnwrapped, bUnwrapped);



            // Если это тяжелая математика (сложение/умножение шумов)
            // и она еще НЕ обернута в маркер Interpolated
//            if (!(function instanceof DensityFunctions.MarkerOrMarked marker && marker.type() == DensityFunctions.Marker.Type.Interpolated)) {
//                return new DensityFunctions.Marker(DensityFunctions.Marker.Type.Interpolated, unwrapped);
//            }
         ...
         */
        if (unwrapped instanceof FastRangeChoice rc) {
        ...
        if (unwrapped instanceof FastShiftedNoiseDensityFunction sn) {
        ...
        if(unwrapped instanceof DensityFunctions.Noise noise) {
            if (noise.yScale() == 0.0) {
                if (!(function instanceof DensityFunctions.MarkerOrMarked marker && marker.type() == DensityFunctions.Marker.Type.Cache2D)) {
                    return new DensityFunctions.Marker(DensityFunctions.Marker.Type.Cache2D, unwrapped);
                }
            }
        }
        ...
        if (unwrapped instanceof DensityFunctions.Spline spline) {
            ...
            if(spline.spline() instanceof CompactSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> coordinateCompactSpline) {
                boolean coordDependsOnY = checkDirectYDependency(coordinateCompactSpline.coordinate());

                if (!coordDependsOnY) {
                    // 2D Сплайн -> Плоский кэш на весь чанк (вычисляется 256 раз)
                    if (coordinateCompactSpline.locations().length > 4) {
                        return new DensityFunctions.Marker(DensityFunctions.Marker.Type.Cache2D, unwrapped);
                    }
                }
            }

            if(spline.spline() instanceof CubicSpline.Constant<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> constant)
                return new DensityFunctions.Constant(constant.value());
        }

//        if (unwrapped instanceof DensityFunctions.Mapped mapped && mapped.type() == DensityFunctions.Mapped.Type.SQUEEZE) {
//            if (!(function instanceof DensityFunctions.MarkerOrMarked marker && marker.type() == DensityFunctions.Marker.Type.Interpolated)) {
//                return new DensityFunctions.Marker(DensityFunctions.Marker.Type.Interpolated, unwrapped);
//            }
//        }
    ...
    private boolean checkDirectYDependency(ToFloatFunction<?> coordinate) {
        String s = coordinate.toString().toLowerCase();
        return s.contains("y_") || s.contains("y_clamped_gradient");
    }
}
