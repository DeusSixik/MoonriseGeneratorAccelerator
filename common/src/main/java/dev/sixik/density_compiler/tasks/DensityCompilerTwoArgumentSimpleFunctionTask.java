...
public class DensityCompilerTwoArgumentSimpleFunctionTask extends
    ...
    protected void applyStep(DCAsmContext ctx, DensityFunctions.TwoArgumentSimpleFunction node, Step step) {
        ...
        if (isConst(arg1) && isConst(arg2)) {
            ...
            ga.push(result); // GeneratorAdapter сам выберет LDC2_W или DCONST
            return;
        }

        // 2. Identity & Zero Logic (Нейтральные элементы)
        ...
        if (type == DensityFunctions.TwoArgumentSimpleFunction.Type.MUL) {
            ...
            if (arg1 == arg2 || arg1.equals(arg2)) {
                ctx.readNode(arg1, Step.Compute); // Stack: [val]
                ga.dup2();                        // Stack: [val, val] (dup2 т.к. double занимает 2 слота)
        ...
        switch (type) {
            ...
            case MIN -> ga.invokeStatic(MATH_TYPE, MATH_MIN);
            case MAX -> ga.invokeStatic(MATH_TYPE, MATH_MAX);
        }
    }
}
